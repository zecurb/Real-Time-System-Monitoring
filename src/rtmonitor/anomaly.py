...
# Minimum expected noise prevents nearly-flat baselines from amplifying harmless
# changes into incidents. Values are expressed in each metric's native unit.
MINIMUM_DISPERSION = {
    "system.load.1m": 0.10,
    "system.load.5m": 0.10,
    "system.load.15m": 0.10,
    "system.process.count": 5.0,
    "memory.used.percent": 1.0,
    "disk.used.percent": 0.5,
}
...
def robust_anomaly_score(
    ...
    *,
    ...
    measured_dispersion = float(median(abs(point - baseline) for point in history))
    dispersion = max(measured_dispersion, minimum_dispersion)
    score = (
        0.0
        if dispersion == 0
        else abs(ROBUST_Z_SCALE * (value - baseline) / dispersion)
    )
